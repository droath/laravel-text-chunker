 ...
 *
 * This strategy encodes text into tokens, chunks the token arrays, then
 * decodes back to text.
 *
 * It supports different OpenAI models (gpt-4, gpt-3.5-turbo,
 * etc.) and provides accurate character position tracking in the original
 * text.
 */
class TokenStrategy implements ChunkerStrategyInterface
{
     ...
     * @param array<string, mixed> $options Options including 'model' (default:
     *     gpt-4)
     *
    ...
    /**
     * Calculate character position in the original text for a given token
     * position.
     *
     * @param \Yethee\Tiktoken\Encoder $encoder The encoder instance
     * @param array<int, int> $tokens All tokens from the original text
     * @param int $tokenPosition The token position to find a character
     *     position for
     *
